...
public class LuggageItemRenderer extends BlockEntityWithoutLevelRenderer {
	...
	public void renderByItem(ItemStack stack, ItemTransforms.TransformType type, PoseStack ms, MultiBufferSource buffer, int light, int overlay) {
		if (stack.is(Registries.ItemRegistry.LUGGAGE.get())) {
			...
			if (entity != null) {
				...
				this.renderEntity(entity, type, ms, light);
			}
	...
	private void renderEntity(Entity entity, ItemTransforms.TransformType type, PoseStack stack, int light) {
		...
		RenderSystem.runAsFancy(() ->
				dispatcher.render(entity, 0.0D, 0.0D, 0.0D, 0.0F, partialTicksForRender, stack, source, type == ItemTransforms.TransformType.GUI ? 15728880 : light));
		source.endBatch();
