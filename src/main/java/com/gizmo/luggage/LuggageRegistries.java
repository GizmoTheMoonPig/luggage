...
public class LuggageRegistries {
	...
	public static final DeferredRegister<DataComponentType<?>> COMPONENTS = DeferredRegister.createDataComponents(LuggageMod.ID);
	...
	public static final DeferredHolder<DataComponentType<?>, DataComponentType<Unit>> EXTENDED = COMPONENTS.register("extended", () -> DataComponentType.<Unit>builder().persistent(Unit.CODEC).networkSynchronized(StreamCodec.unit(Unit.INSTANCE)).build());
	...
	private static DeferredHolder<SoundEvent, SoundEvent> createEvent(String sound) {
		ResourceLocation name = ResourceLocation.fromNamespaceAndPath(LuggageMod.ID, sound);
